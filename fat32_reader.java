...
public class fat32Reader {
    ...
    public ArrayList<Integer> getBytesArray(String fat32Img,int offset) throws IOException {
        ...
        while(!done) {
            boolean zero = true;
            for (int i = x; i < x + 32 && (zero || i < x + 11); i++) {
                //Covert number to unsigned if negative.
                if(i < x + 11) {
                    byteArray.add(out.get(i) & 0xFF);
                }
                if(zero){
                    if(out.get(i) != 0){
                        zero = false;
